...
function countUp(elementId, target) {
	...
	const interval = setInterval(() => {
		...
		if (current >= target) {
			element.textContent = target + " +"; // Set the final value with plus sign
...
events.forEach((event) => {
	event.addEventListener("mouseover", () => {
		events.forEach((a) => a.classList.add("shrink"));
		event.classList.remove("shrink");
	});

	event.addEventListener("mouseout", () => {
		events.forEach((c) => c.classList.remove("shrink"));
	});
});
// ##################################################################
